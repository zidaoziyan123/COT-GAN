...
parser.add_argument('-sinkl', '--sinkhorn_l', type=int, default=10000)
...
def main():
    ...
    if p_filter_size > 0:
        projector = gan.RandomLinearProjector(p_filter_size)
    else:
    ...
    model_fn = "%s_Dz%d_Dy%d_Dx%d_bs%d_gss%d_gfs%d_dss%d_dfs%d_pfs%d_ts%d_r%d_eps%d_l%d_lr%d_nl%d_s%02d" % (
        dname, z_dims_t, y_dims, Dx, batch_size, g_state_size, g_filter_size, d_state_size, d_filter_size,
        p_filter_size, time_steps, np.round(np.log10(reg_penalty)), np.round(np.log10(sinkhorn_eps)),
    ...
    def disc_training_step(real_data, real_data_p):
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            f_real = projector.call(real_data)
            ...
            f_real_p = projector.call(real_data_p)
    ...
    with tqdm.trange(n_iters, ncols=150) as it:
        for _ in it:
            ...
            if isinstance(projector, gan.RandomLinearProjector):
                projector.update()
