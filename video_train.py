...
parser.add_argument('-pfs', '--p_filter_size', type=int, default=0)
...
parser.add_argument('-p', '--path', type=str, default='data/*_data.tfrecord')
...
def main():
    ...
    p_filter_size = args.p_filter_size
    ...
    if projection:
        projector = gan.VideoLinearProjector(batch_size, time_steps, x_width, x_height, filter_size=p_filter_size,
                                             channels=channels)
    else:
        projector = gan.IdentityDiscriminator()
    ...
    model_fn = "%s_Dz%d_Dy%d_bs%d_gss%d_gfs%d_dss%d_dfs%d_pfs%d_eps%d_l%d_p%d_lr%d_nl%d_s%02d" % \
               (dname, args.z_dims_t, args.y_dims, batch_size, g_state_size, g_filter_size, d_state_size, d_filter_size,
                p_filter_size, np.round(np.log10(sinkhorn_eps)), sinkhorn_l, np.round(np.log10(reg_penalty)),
    ...
    def disc_training_step(real_x, real_x_p):
        ...
        with tf.GradientTape() as disc_tape:
            ...
            f_real = projector.call(real_x)
            f_fake = projector.call(fake_data)
            ...
            f_real_p = projector.call(real_x_p)
            f_fake_p = projector.call(fake_data_p)
            ...
            f_fake = tf.reshape(f_fake, [batch_size, time_steps, -1])
            f_fake_p = tf.reshape(f_fake_p, [batch_size, time_steps, -1])

            f_real = tf.reshape(f_real, [batch_size, time_steps, -1])
            f_real_p = tf.reshape(f_real_p, [batch_size, time_steps, -1])

            loss1 = gan_utils.compute_mixed_sinkhorn_loss(f_real, f_fake, m_real, m_fake, h_fake, scaling_coef,
                                                          sinkhorn_eps, sinkhorn_l, f_real_p, f_fake_p, m_real_p,
                                                          h_real_p, h_fake_p)
    ...
    def gen_training_step(real_x, real_x_p):
        ...
        with tf.GradientTape() as gen_tape:
            ...
            loss2 = gan_utils.compute_mixed_sinkhorn_loss(f_real, f_fake, m_real, m_fake, h_fake, scaling_coef,
                                                          sinkhorn_eps, sinkhorn_l, f_real_p, f_fake_p, m_real_p,
                                                          h_real_p, h_fake_p)
